...
float m_pow_float(float base, float exponent) {
    ...
    if (floor(exponent) == exponent) {
        // For integer exponents, use the optimized integer power function
        if (exponent >= 0.0f) {
    ...
    // Handle negative base for non-integer exponents
    if (base < 0.0f) {
        return NAN;
    }
    
    // Use standard library's pow for floating-point exponents
...
UNARY_OP(float, exp10_f32, pow(10.0f, x));
...
UNARY_OP(half, relu_f16, x > 0.0h ? x : 0.0h);
UNARY_OP(half, sigmoid_f16, half(1.0h / (1.0h + exp(float(-x)))));
...
UNARY_OP(half, tan_f16, half(tan(float(x))));
...
UNARY_OP(half, exp10_f16, half(pow(10.0f, float(x))));
UNARY_OP(half, exp2_f16, half(exp2(float(x))));
UNARY_OP(half, softplus_f16, half(log(1.0f + exp(float(x)))));
UNARY_OP(half, recip_f16, 1.0h / x);

UNARY_OP_OUTPUT(half, bool, logical_not_f16, x == 0.0h);
...
UNARY_OP(bfloat, tan_bf16, bfloat(tan(float(x))));
...
UNARY_OP(bfloat, exp10_bf16, bfloat(pow(10.0f, float(x))));
UNARY_OP(bfloat, exp2_bf16, bfloat(exp2(float(x))));
UNARY_OP(bfloat, softplus_bf16, bfloat(log(1.0f + exp(float(x)))));
UNARY_OP(bfloat, recip_bf16, 1.0bf / x);

UNARY_OP_OUTPUT(bfloat, bool, logical_not_bf16, x == 0.0bf);
...
UNARY_OP_WITH_CONSTANT(uint8_t, uint8_t, pow_u8, m_pow_int(x, const_val));
UNARY_OP_WITH_CONSTANT(uint16_t, uint16_t, pow_u16, m_pow_int(x, const_val));
UNARY_OP_WITH_CONSTANT(uint32_t, uint32_t, pow_u32, m_pow_int(x, const_val));
UNARY_OP_WITH_CONSTANT(int8_t, int8_t, pow_i8, m_pow_int(x, const_val));
UNARY_OP_WITH_CONSTANT(int16_t, int16_t, pow_i16, m_pow_int(x, const_val));
UNARY_OP_WITH_CONSTANT(int32_t, int32_t, pow_i32, m_pow_int(x, const_val));
...
UNARY_OP_WITH_CONSTANT(half, half, maximum_scalar_f16, maximum(x, const_val));
UNARY_OP_WITH_CONSTANT(half, half, minimum_scalar_f16, minimum(x, const_val));
UNARY_OP_WITH_CONSTANT(half, half, pow_f16, half(pow(float(x), float(const_val))));
UNARY_OP_WITH_CONSTANT(half, half, leaky_relu_f16, x > 0.0h ? x : const_val * x);
UNARY_OP_WITH_CONSTANT(half, half, elu_f16, half(x > 0.0h ? x : float(const_val) * (exp(float(x)) - 1.0h)));
...
UNARY_OP_WITH_CONSTANT(bfloat, bfloat, maximum_scalar_bf16, maximum(x, const_val));
UNARY_OP_WITH_CONSTANT(bfloat, bfloat, minimum_scalar_bf16, minimum(x, const_val));
UNARY_OP_WITH_CONSTANT(bfloat, bfloat, pow_bf16, bfloat(pow(float(x), float(const_val))));
UNARY_OP_WITH_CONSTANT(bfloat, bfloat, leaky_relu_bf16, x > 0.0bf ? x : const_val * x);
UNARY_OP_WITH_CONSTANT(bfloat, bfloat, elu_bf16, bfloat(x > 0.0f ? x : float(const_val) * (exp(float(x)) - 1.0f)));

UNARY_OP_WITH_CONSTANT(bfloat, bool, eq_scalar_bf16, x == const_val);
UNARY_OP_WITH_CONSTANT(bfloat, bool, ne_scalar_bf16, x != const_val);
UNARY_OP_WITH_CONSTANT(bfloat, bool, lt_scalar_bf16, x < const_val);
UNARY_OP_WITH_CONSTANT(bfloat, bool, le_scalar_bf16, x <= const_val);
UNARY_OP_WITH_CONSTANT(bfloat, bool, gt_scalar_bf16, x > const_val);
UNARY_OP_WITH_CONSTANT(bfloat, bool, ge_scalar_bf16, x >= const_val);
