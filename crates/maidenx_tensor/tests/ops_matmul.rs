...
mod test_functions {
    ...
    pub fn vector_vector_test(dtype: DType) -> Result<()> {
        match dtype {
            DType::U8 | DType::U16 | DType::U32 | DType::U64 | DType::I8 | DType::I16 | DType::I32 | DType::I64 => {
                let a = setup_tensor(vec![1u32, 2, 3], dtype)?;
                let b = setup_tensor(vec![4u32, 5, 6], dtype)?;
                let c = a.matmul(&b)?;
                assert_eq!(c.ndim(), 0);
    ...
    pub fn matrix_vector_test(dtype: DType) -> Result<()> {
        match dtype {
            DType::U8 | DType::U16 | DType::U32 | DType::U64 | DType::I8 | DType::I16 | DType::I32 | DType::I64 => {
                let matrix = setup_tensor(vec![vec![1u32, 2, 3], vec![4, 5, 6]], dtype)?;
                let vector = setup_tensor(vec![2u32, 1, 3], dtype)?;
    ...
    pub fn vector_matrix_test(dtype: DType) -> Result<()> {
        match dtype {
            DType::U8 | DType::U16 | DType::U32 | DType::U64 | DType::I8 | DType::I16 | DType::I32 | DType::I64 => {
                let vector = setup_tensor(vec![2u32, 3], dtype)?;
                let matrix = setup_tensor(vec![vec![1u32, 2, 3], vec![4, 5, 6]], dtype)?;
    ...
    pub fn matrix_matrix_test(dtype: DType) -> Result<()> {
        match dtype {
            DType::U8 | DType::U16 | DType::U32 | DType::U64 | DType::I8 | DType::I16 | DType::I32 | DType::I64 => {
                let a = setup_tensor(vec![vec![1u32, 2], vec![3, 4]], dtype)?;
                let b = setup_tensor(vec![vec![5u32, 6], vec![7, 8]], dtype)?;
                let c = a.matmul(&b)?;
    ...
    pub fn batched_matrix_matrix_test(dtype: DType) -> Result<()> {
        match dtype {
            DType::U8 | DType::U16 | DType::U32 | DType::U64 | DType::I8 | DType::I16 | DType::I32 | DType::I64 => {
                let a = setup_tensor(vec![vec![vec![1u32, 2], vec![3, 4]], vec![vec![5, 6], vec![7, 8]]], dtype)?;
                let b = setup_tensor(vec![vec![vec![9u32, 10], vec![11, 12]], vec![vec![13, 14], vec![15, 16]]], dtype)?;
                let c = a.matmul(&b)?;
